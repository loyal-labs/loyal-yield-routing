...
function wrapper(policy: string, inner: readonly Instruction[], constraintIndices: readonly number[]): Instruction {
  const result = runRustCompiler<{ schema?: unknown; instruction?: ReturnType<typeof innerWire> }>({
    compilerBinary: "compile-voltr-custom-execution",
    cwd: ROOT,
    input: JSON.stringify({ policy, delegatedSigner: RWA_MULTIPLY_ROUTE.squads.delegatedExecutor, accountIndex: 0, constraintIndices, inner: inner.map(innerWire) }),
    maxBuffer: 16 * 1024 * 1024,
    label: "bridge wrapper compiler",
  });
  compilerProvenance = result.compiler;
