...
try {
  ...
  if(!directory||!/^\/private\/tmp\/backyard-(?:phase3|selector)-kamino-probe\.[A-Za-z0-9]+$/.test(directory)||!process.env.SOLANA_RPC_URL)throw new Error();
  const plan=JSON.parse(readFileSync(resolve(directory,"plan.json"),"utf8"));
  const historical=plan.schema==="phase3-kamino-controlled-probe/v1"&&plan.lane==="Ethena/USDe/PYUSD";
  const selector=plan.schema==="selector-kamino-position-probe/v1"&&plan.lane==="Maple/syrupUSDC/USDC"&&Number.isSafeInteger(plan.sizingSlot)&&plan.sizingSlot>0;
  if((!historical&&!selector)||plan.broadcast!==false||plan.addresses.length>100)throw new Error();
  ...
  let minSlot=Math.max(await rpc.getSlot("finalized"),selector?plan.sizingSlot:0);
  const identities=[];
  for(const program of programs){
    ...
    const file=program+".so";const target=resolve(directory,file);
    if(existsSync(target)){if(sha(readFileSync(target))!==sha(elf))throw new Error();}
    else writeFileSync(target,elf,{flag:"wx",mode:0o600});
...
}catch{console.error(`Read-only Kamino probe snapshot unavailable or identity changed: ${stage}`);process.exitCode=1;}
