...
## Native initializer runtime integration

Initializer preparation now measures exact-message fees and current rent, then checks native funding and installed-policy/metadata/market/absent-obligation state. Decision production requires active validated pilot authority, complete initializer manifest bindings, a healthy entry market and no prior token exposure, working cash, withdrawal demand or recovery work. The shared budget admission records native gross rent plus fee with a measured fee-only execution expense and cannot consume a prior exit reserve. The production worker records the decision before admission/build and retains the existing signed-wire/finality machinery.

Full Go/database tests and vet pass. Local tests exercise native RPC preparation, creation/withdrawal races, measured PostgreSQL admission/retry/build and exact dispatch ordering. Fable reviewed the connected path and found no blocker; no additional live execution is claimed. A withdrawal arriving after preparation can coexist with this native-only operation; no user USDC is allocated by initialization. The embedded initializer policy list is still empty and remains a live activation gate.
