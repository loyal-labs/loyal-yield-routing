...
**Version:** `earn-max-v2`
**Date:** 2026-08-23
**Status:** Authoritative implementation and release contract. This replaces the old server-prepared and direct-database-mutation contract. The unchanged on-chain policy manifest remains `earn-max-v1`.
...
Objective:
  ...
  The browser creates and submits the user's transactions. Confirmed Solana
  transactions and accounts are projected by the existing LaserStream owner.
...
Scope:
  ...
  manifest earn-max-v1, and syrupUSDC/USDC. Mainnet at confirmed commitment.
  USX is excluded. No SVM or fixture proof, hook, guard, flash loan, pooled
...
+ multiply_operations + multiply_position_snapshots
             |
             v
three authenticated read endpoints -> one Earn MAX UI adapter
```

There is no application write path. Solana owns policy existence, user intents,
token balances, and protocol balances. LaserStream is transport; exact confirmed
transaction/account readback is proof. Neon stores only current policy/route
state, literal operations, performance snapshots, and the existing durable
projection cursor.
...
never signs user deposits, requests, cancels, claims, or closes. It observes
confirmed deposits/claims and reconciles exact token deltas. A top-up is simply
another confirmed deposit; any idle claim balance is deployed even when an
active obligation already exists.
...
The deployed slice has exactly three authenticated, write-free endpoints:

```text
GET /api/smart-accounts/earn-max/state
GET /api/smart-accounts/earn-max/performance
GET /api/smart-accounts/earn-max/activity
```

There are no prepare, confirmation, withdrawal mutation, policy mutation, or
generic transaction endpoints. The state response exposes only read-only client
...
One Earn MAX view model adapts those three responses into:
...
2. **Cheap architecture:** one projector, four existing tables, three GETs, one
   client adapter; no forbidden program, endpoint, table, direct mutation, mock,
   fixed user identity, hook, guard, flash loan, USX, or second authority.
...
7. **Accounting/API truth:** deployed state, performance, and activity recompute
...
authorized confirmed lifecycle plus replay are complete. Do not generalize to a
second strategy before this one passes literally.
