...
The 1-USDC user deposit remains idle. Worker `5e8d759` is live and its first refreshed NAV report reconciled at slot 447918956. All three unsigned valuation captures fit, and the archived activation validates without changing authority or spending history. The exact 4652-LP withdrawal request passes signed simulation; it has not been sent.

The second one-use Maple canary was accepted at 23:23:11Z, but its quote expired before allocation construction finished. Quote slots 447919013–447919045; subsequent hold observed slot 447919058. No allocation was sent. Preserve both consumed IDs. The reviewed latency fix now carries the coherent account batch within the same tick and wakes the serialized worker immediately after selector acceptance. Full database-backed Go tests and vet pass; live read-only checks validate all three valuation captures and the original activation record. Original freshness, budget, simulation, durable binding, latch and send checks remain. Deploy this candidate and use a fresh bounded canary; no quote deadline extensions or history resets.
...
| Release scope | Worker `5e8d759` passed Actions 35286388331 and is live with matching lease; frontend `0b23bb5f` is Ready as `dpl_DWhZuun8zr6erjMVCi6cCwUXFN6S` with matching worker pin | Complete funded acceptance on the final candidate |
