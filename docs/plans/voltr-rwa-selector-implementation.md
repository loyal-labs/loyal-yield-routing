...
**Immediate release fixes:** cleanup/baseline release `f503416` passed CI run 35277949861 and is live on replacement service as `dep-dam5ucbijhts73a2kic0`. Frontend `f9df7346` is Ready as `dpl_2He4cvXpTZrQmXyPLkSZTc6JcFoQ`; both APIs return HTTP 200. An operator holds the normal fenced route lease, preventing worker actions until the economic-unit correction deploys. The collector emits raw debt amounts; the selector incorrectly multiplied them by 1e6. The correction preserves raw units and verifies that a 100,000-USDC borrow against 1,000,000-USDC supply moves utilization from 50% to 60%. No threshold/capacity guard is relaxed. Shadow `activationBlockers` are stale static text, not live admission results; the live adapter independently quotes capacity.
