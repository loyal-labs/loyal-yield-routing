...
The 1-USDC user deposit remains idle. Worker `f46e9d3` is deployed, owns the route lease and is recognized by the deployed frontend. The same-tick coherent batch and immediate serialized wake are live. Three canary IDs remain consumed and preserved; no allocation has been signed or sent.

The third Maple canary reached durable allocation admission, then expired during unsigned build at 23:39:41Z. Quote slots 447922127–447922159; acceptance 23:39:34Z. The remaining dominant delay is serial independent exact-message fee and price reads in quote preparation, admission and build. The reviewed patch runs these with bounded concurrency, keeping each original source slot and the minimum expiry, then rejects missing, future or expired evidence after joining all reads. Live read-only quote collection still finds the executable 1-USDC Maple route; all prior canary attempts and spending history remain unchanged. No quote extensions, spending resets or old-ID retries. Claim tooling is prepared for coordinator review; request and claim remain unsent.
...
| Release scope | Worker `f46e9d3` passed Actions 35287527358 and is live with matching lease; frontend `0b23bb5f` is Ready as `dpl_GNmezyKsQbEKev8rVSZNhoGyDuzH` with matching worker pin | Complete funded acceptance on the final candidate |
