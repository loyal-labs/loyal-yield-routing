...
The 1-USDC deposit has been allocated, swapped to syrupUSDC, and deposited as Kamino collateral. Borrowing has not started. Release `19cfe2be6663fee8b757e54146b8e9e8fe934a0b` passed CI 35290222414 and is live on the single-instance starter-plan Virginia replacement `srv-dam87bp42hec738loc6g`. Oregon service `srv-dalootgae00c73c4qhag` and historical `srv-dabkt0ojo6nc7381o9fg` remain suspended. The shared database, activation, budget, journal and four consumed canary IDs are preserved.

**Cleared blockers:** migration81 applied and passed the existing catalog validation. The new worker automatically settled finalized failed NAV `n1jg9BQi9sTapcR7pobxoMyqkHLsXCrH6jc8DHGGuS59pYWsQ2fjtDhFuQwwCKjeGYm4S1X5ZZ1HrqoUAgQ9qxA`, preserving its wire and finalized receipt, charging the 5000-lamport fee at the retained conservative valuation of 519 micros, and releasing only its reconciled reservation. Fresh NAV resumed. Planning now uses one fenced per-observation state read; no cross-tick cache or expiry extension. Region placement was Oregon versus Neon us-east-1; moving to Virginia removes that cross-country latency from roughly 70 serial database exchanges.

**Financed acceptance:** fifth canary `af00224c79ae6f9f247ec57537ee8da9ec5dc5ce85a894c43d28c5ee10a58358` was accepted at 00:23:51.223Z; allocation submitted at 00:23:52.563Z and finalized at 447930511. Swap finalized at 447930587, NAV at 447930656, and collateral deposit at 447930732. All four exact landed wires/signatures and public balance changes independently verified in `financed-entry-finalized.json`. The four earlier attempts remain consumed and never allocated.

**Current blocker:** post-deposit NAV is classified as recovery and fails before signing because current fee 516 micros + fresh exit 4085046 micros exceeds prior reserve 4085553 micros by 9 micros. Treat a non-unwind maintenance report as fee spending under existing caps while retaining at least the entire prior exit reserve. Preserve strict recovery handling for actual withdrawal/risk/unwind reports; do not weaken generic budget rules. Then complete borrow/redeposit/NAV before requesting the exact 4652 LP withdrawal and claim.

Current read-only pair evidence: PRIME and ONyc have zero admissible capacity with borrow-utilization blocked; Maple has capacity. Complete the financed Maple round trip first. Same-release live rotation remains dependent on another eligible lane. Exact 4652-LP request/claim tooling is reviewed, and claim recovery uses finalized USDC conservation and receipt bounds. Request and claim remain unsent.

### Current implementation ownership

- Coordinator: current goal, production state, review, credentials, deployment, budget and all financial actions. Pilot budget generation 819 is activated after finalized cleanup; the Virginia worker owns the fenced lease, settled the failed NAV fee and resumed fresh reports.
...
| Release scope | Worker `19cfe2b` passed Actions 35290222414 and is live in Virginia with matching lease; frontend `0b23bb5f` is Ready as `dpl_Hot5fS4BMHkqjhWW9LYqxuTpvRzK` with matching worker and service pins | Complete funded acceptance on the final candidate |
| Chain/database activation | 100-USDC cap finalized at 447724467; policies 149–151 finalized and pinned; migrations 74–81 applied and validated | Cleared: return finalized at 447899062; pilot budget generation 819 activated at 447899153 |
...
**Current runtime and client:** worker `19cfe2b` is live on Virginia replacement service `srv-dam87bp42hec738loc6g`; historical service remains suspended. The manifest hash is `54dfdce596a237ef0c3726f20aa999ffd61152fdc3dcfb30608227fd975d68b0`. The first production NAV report reconciled at slot 447903813, signature `4cmpa767RLTZwJwB11XHjN4b9QnQCN3bmY1ktn2a9BWUBU3PauitSRpz9kWBJuwfwRBsXZ14yWetnR1ntcZbxDBa`. The client is deployed as `dpl_Hot5fS4BMHkqjhWW9LYqxuTpvRzK` with the Virginia service and image pins; served readiness verification follows the canary restart. Deposits remain disabled.
...
**Current activation work:** complete. The approved 214898 raw PYUSD conversion yielded 214921 raw USDC. All three accounting-aware bridge steps finalized: report, stage, restore. HXtk idle and book both equal 214944 raw USDC; receipt NAV, tracked custody, manager and strategy cash are zero. Strict finalized flat validation passed. Pilot budget generation 819 was activated at finalized slot 447899153 without resetting history. Public receipts are in `pilot-cleanup-return-finalized.json`; signed intents remain private. The replacement worker has resumed on the reviewed release; the historical worker remains suspended.

**Immediate next milestone:** the first 1-USDC user deposit finalized at slot 447907948, signature `2PruLGsNs5ioNbS2uFmVMRNf593B4Em9cknZ9EP1gqHN4Dr1JQcg4fMNA3kTJafDdqMtScTxtcGpsuQtbTFKoP2Z`. Exact wallet USDC debit, vault idle credit, 4652 raw LP minted and 5000-lamport fee were independently verified; the deployed app returns `finalized-reconciled`. Funds are idle and external deposits remain disabled. The GLM reserve-refresh quote fix includes the required readonly account graph in one unsigned simulation. Live destination probes now quote syrupUSDC at 1-USDC equity with 497523 raw borrow; PRIME and ONyc currently fail capacity checks. Full financed loop and withdrawals remain unproven.

**Client report freshness:** frontend `0b23bb5f` is Ready as `dpl_AqAJ3BPt6tNWawCMYE9r2KScpmxm`. It preserves the adaptor's 32-slot consume deadline and separately applies the worker's existing 60-second NAV age using the matched report transaction, receipt timestamp and finalized chain clock. A live read recognized an 11-second-old report after finality. 112 report checks, 33 service checks, typecheck and lint pass.

**Flat Maple unwind reports before completing (2026-09-25):** on the fixed-lane decision path (`decide.go`), a flat committed unwind with unreported capital mutation now reports `withdrawal_terminal_nav_due` before `unwind_complete`, mirroring the debt-lane path. Before, it held `unwind_complete` forever: completion requires `!CapitalMutated`, and only a report clears that. The first Maple economic-rotation unwind hit this; earlier Maple unwinds were withdrawal-driven and ended through `withdrawal_covered`.

**Stale-input holds retry in-process (2026-09-25):** a tick that ends only in a hold whose cause is an aged-out or briefly unavailable quote, price or fee reading (`staleInputHoldReasons` in `worker.go`: entry quote expired, stale USDC/native/token valuation, stale funding exit admission, initializer decision changed, network fee unavailable, send valuation expired/slot unavailable, and since fix 22 the pre-send pilot release projection re-checks `pilot_release_projection_expired`/`_risk_changed` and `fee_message_or_slot_mismatch`) now retries on the next tick instead of exiting the process. The next tick re-observes the chain and re-derives the leg, which is what a restart did; the Render restart cost 1–25 s each time (36 restarts on 2026-09-25). Every other hold, and any hold joined with another error, still stops the worker.

**Canary economics and one-use control:** the production selector retains its $0.25 net benefit over seven days, uncertainty margin and full costs. Small acceptance balances correctly remain idle economically. An optional `BACKYARD_RWA_PILOT_CANARY_ENTRY` JSON object (`id`: unique 64-character SHA-256, `lane`: one reviewed lane, `equityRaw`: positive and at most the existing working cap, `expiresAt`: UTC within 15 minutes) requests one explicit `CANARY_ENTER`. It requires the same real complete quote, capacity, current accounting, flat custody, active pilot, no pending work/manual hold/unwind, and all normal transaction budget/simulation/recovery checks. Acceptance consumes the ID atomically with `selectorEntry` under the route fence and keeps its receipt in `pilotCanaryEntries`; restarting never replays it. Since 2026-09-24 one unexpired request may re-accept with a fresh quote when its own entry lapsed before any allocation bound to it (a new lane needs the obligation initializer, then the allocation, and each entry lapses 30 s after its quote); it consumes no new receipt. Since 2026-09-25 the obligation initializer, which moves no principal and whose rent returns on close, needs only the accepted entry's own expiry, not the quote's 32-slot window: quote collection plus the initializer tick overran that window on every attempt. The allocation still requires a current quote slot. Once an allocation binds, or the request's own expiry passes, a new explicit request is needed, preserving prior history and spending. Remove the setting after acceptance to resume economic selection. While a request is installed, quote collection prices only the requested lane; other lanes publish `operator_canary_lane_only` with no executable capacity. For every lane, the size ladder stops pricing smaller sizes once 3 s have passed since the observation (the largest size is always priced), so an accepted quote keeps enough of its 32-slot window for the allocation. Four prior request IDs are consumed and preserved; no allocation was sent. The retained receipt capacity was raised from 16 to 32 on 2026-09-25, after the Maple→AUTO→Maple rotation used all 16. Full local database-backed tests cover consumption across restart, quote/capacity/accounting refusals and unchanged spending history.
