...
**Immediate release fixes:** corrected raw-unit image `90f9aea` passed CI run 35278765755 and deployed as `dep-dam61vm7bikc738f5o0g`. Frontend `f9df7346` is Ready as `dpl_FvAkJ3Wk6B8NBtDZL99yfiek9rZP` with the matching image pin and deposits disabled. The worker owns the correct lease and reads the validated activation baseline. Its first NAV attempts are durably refused before broadcast as `report_stale`: the five-second polling sleep after durable signing consumes the report window. The next patch immediately reloads the just-signed operation, rechecks manual stop, and calls the existing fenced single-send/recovery path; it preserves the 28-slot send fence and 32-slot on-chain limit. Full Go tests including isolated database cases and vet pass. No production NAV report has reconciled yet; funded acceptance is still pending.
