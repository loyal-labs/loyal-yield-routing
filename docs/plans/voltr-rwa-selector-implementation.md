...
| Public access | Client gate now checks current release lease, pilot authority, pending work, report identity, NAV/custody, freshness and cap; default remains closed; hosted API verifies the expected vault and 100-USDC cap at slot 447736158; typecheck/lint and 33 controlled service cases pass | Finish the current client deployment, complete real wallet journey, then enable |
...
**Publication authorized:** the user explicitly approved publishing the pending implementation and rollout notes to the existing Loyal GitHub repositories. Frontend `3d15b413` and provisioning note `368575fe` are pushed. Deployment `dpl_8CzMsfnytBUwuEivcrJkzbd4rAQR` is in progress.

**Current activation work:** the user explicitly confirmed that the 214898 raw PYUSD is Loyal-owned test money and authorized conversion to USDC and return to HXtk. Fresh finalized custody at slot 447889670 confirms that exact balance. Other shared-wallet tokens remain outside this authority. GLM task 616 prepares the existing cleanup execution path; the coordinator owns signing, submission and reconciliation. Convert/return with accounting intact, then activate the history-preserving pilot budget and resume the pinned replacement worker.
