...
The 1-USDC user deposit remains idle. Worker `0e81561` and matching frontend are deployed. Same-tick coherent bridge evidence, immediate serialized wake and bounded parallel fee/price reads are live. Four canary IDs remain consumed and preserved; no allocation has been signed or sent.

The fourth canary reached allocation decision at 23:51:38.601Z, almost six seconds after acceptance (23:51:32.687Z). Its quote began at slot 447924407 and expired at 447924439; decision valuation was already 447924435. Admission then held at 23:51:40.627Z. The deployed RPC matches the locally tested endpoint. The remaining critical path repeatedly reads the same route state and activation across planning. Consolidate this into one validated per-observation state read, reuse it for lane choice and enrichment, and reject any generation change under the existing projection lock before returning an actionable observation. Shadow collection must preserve its existing generation/lease fence. Never cache across ticks or extend quote expiry.

**Highest-impact latency fix:** verified worker region is Oregon and Neon region is `us-east-1`. Roughly 70 sequential database exchanges make the cross-region placement a structural deadline risk. Move this pilot to a same-plan, single-instance Virginia replacement after migration81 and the reviewed image pass. Suspend the Oregon service before the replacement can acquire the same fenced lease; preserve the shared database, budget, journal and all consumed IDs. Copy only required runtime configuration in memory, omit the consumed canary setting, and verify recovery plus fresh NAV before creating the next bounded canary. Keep immutable GHCR images and update the frontend service/image pins. Do not extend protocol deadlines.

Current read-only pair evidence: PRIME and ONyc have zero admissible capacity with borrow-utilization blocked; Maple has capacity. Complete the financed Maple round trip first. Same-release live rotation remains dependent on another eligible lane. Claim tooling is prepared for review; request and claim remain unsent.

A separate recovery blocker is active: NAV signature `n1jg9BQi9sTapcR7pobxoMyqkHLsXCrH6jc8DHGGuS59pYWsQ2fjtDhFuQwwCKjeGYm4S1X5ZZ1HrqoUAgQ9qxA` finalized failed at slot 447924535, 33 slots after its report observation. Only the 5000-lamport payer fee changed; token balances are unchanged. The old recovery path tries to release it as unspent, which migration75 correctly disallows for a broadcast failure. Add exact finalized-failure proof and conservative fee settlement against the existing reservation, with narrowly guarded schema support. Missing proof must preserve the reservation. Do not clear or rewrite this operation manually.

### Current implementation ownership

- Coordinator: current goal, production state, review, credentials, deployment, budget and all financial actions. Pilot budget generation 819 is activated after finalized cleanup; the current worker is crash-looping on the finalized failed NAV recovery; repair and reconcile it before the next canary.
...
| Release scope | Worker `0e81561` passed Actions 35288407179 and is live with matching lease; frontend `0b23bb5f` is Ready as `dpl_4wzykJ2fLLyghegfDLjJ3bjMBePh` with matching worker pin | Complete funded acceptance on the final candidate |
...
**Current runtime and client:** worker `0e81561` is live on replacement service `srv-dalootgae00c73c4qhag`; historical service remains suspended. The manifest hash is `54dfdce596a237ef0c3726f20aa999ffd61152fdc3dcfb30608227fd975d68b0`. The first production NAV report reconciled at slot 447903813, signature `4cmpa767RLTZwJwB11XHjN4b9QnQCN3bmY1ktn2a9BWUBU3PauitSRpz9kWBJuwfwRBsXZ14yWetnR1ntcZbxDBa`. The deployed client worker API returns HTTP 200, but currently reports the stale observation/missing lease caused by NAV recovery. Deposits remain disabled.
