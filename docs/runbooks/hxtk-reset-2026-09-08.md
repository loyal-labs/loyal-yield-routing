...
new-journal re-arm instruction; it never sends or enters policy removal. A
new-journal `--execute` has the same publication barrier: while holding the
claim it completes and validates the original proof-bearing artifact first,
then the canonical marker. If either step fails, it refuses nonzero with the
original-journal reconcile instruction and never builds, elects, or sends a
new attempt. Only after the original record has been verified with the same
attempt token, both expiry heights, and both absent reads may it elect the new
attempt; the original journal remains history.
...
| `aborted-pre-send` (`attempted-expired-proven`) | abort artifact present, original journal | Under the claim, completes the artifact then marker idempotently and verifies the same attempt token, both expiry heights, and both absent reads; then refuses nonzero with the new-journal re-arm instruction. It never sends or enters policy removal. | Completes the artifact then marker idempotently and returns 0; otherwise reports the proven state. | Reports both heights and both absent reads; writes nothing. | Handles only the independent claim. |
| `aborted-pre-send` (`attempted-expired-proven`) | new journal | Under the claim, first completes and verifies the original attempt's artifact then marker. On any completion failure, refuses nonzero with the original-journal reconcile instruction and never builds or sends. After validation, elects a new attempt token/pending generation and preserves the original journal in history; never reuses the original journal. | Reports the proven expiry and new-journal instruction; sends nothing. | Reports re-armable state; writes nothing. | Handles only the independent claim. |
