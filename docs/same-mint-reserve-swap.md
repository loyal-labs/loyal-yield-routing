...
The provisioner is dry-run/reconcile-first and uses the standard
`POLICY_KEYPAIR` only for an explicit, budgeted `--execute` run. Its lamport
ceiling is a durable PostgreSQL-backed cluster-wide rolling window, so restarts
and overlapping provisioners cannot reset it. The family authority and payer
must both match that policy public key. Normal same-mint,
idle-vault, policy, cleanup, monitor, and E2E movement never create or extend a
table. Shared-catalog drift fails before durable movement intent and records a
readiness/repair blocker without allocating vault capacity. Missing vault
coverage fails at the same predecision fence, writes one idempotent provisioning
...
Cleanup requires `SOLANA_RPC_URL` in every mode and verifies its genesis hash against the explicit cluster before any chain read. It protects registry tables, `YIELD_ROUTE_LOOKUP_TABLES`, and explicit `--allowlist` tables. Before the approved legacy refund, ensure `YIELD_ROUTE_LOOKUP_TABLES` is unset so it cannot protect retired old tables. It keeps the JSON report on stdout and logs only the RPC origin; add `--trace-timing` to emit timestamped phase trace logs to stderr. `--include-env-authorities` derives audited public keys from present `YIELD_ROUTER_KEYPAIR`, `POLICY_KEYPAIR`, `DEPLOYMENT_PK`, and `SOLANA_TESTING_PK` values without printing secrets. `--scan-program-accounts` discovers currently reclaimable ALT accounts by authority, with paginated `getProgramAccountsV2` fallback on Helius. `--scan-history` adds recent create/extend/deactivate/close evidence from audited signer history; use `--history-limit <N>` to widen the search and `--min-slot <SLOT>` for post-deploy audits. Limited dry runs stop once the requested candidate limit is reached, and candidate accounts are loaded in batches. Execute refuses limits or partial inventory: it exhaustively rescans the standard-policy fleet and requires the freshly approved `legacyFleetCount` and `inventoryFleetHash`. Active legacy tables are deactivated first, and closeable deactivated tables are closed only after the ALT cooldown. Direct legacy cleanup defaults to the standard `POLICY_KEYPAIR`, and the close recipient must be its public key. Every live cleanup transaction is simulated immediately before submit and verified at finalized commitment; `--bundle-size <N>` groups up to N close/deactivate instructions that share the same authority signer. Stop and drain the old monitor before any deactivation, wait the observed SlotHashes cooldown, repeat the exhaustive zero-reference fleet scan immediately before close, and record finalized RPC absence plus the policy recipient's lamport delta.
...
`SOLANA_TESTING_PK` may pay the outer transaction, while `POLICY_KEYPAIR`
remains the delegated policy signer. Fleet optimization uses
`POLICY_KEYPAIR` as both outer payer and delegated signer, initializes a
...
Full withdraw mode uses `POLICY_KEYPAIR` to execute one policy-mediated
...
Fleet mode uses `POLICY_KEYPAIR` for active-policy discovery and route
...
`--optimization-cycle --execute` first requires a chain preflight with a non-zero source collateral account. If the destination obligation is missing, the decoded Squads route policy-account state must include a target-market `init_obligation` constraint; the script resolves and verifies ALT coverage for that setup transaction, executes it with `POLICY_KEYPAIR`, confirms it, and reloads chain state before route planning continues. The decoded policy must also include `POLICY_KEYPAIR` as a delegated signer, both required markets, and the required live KLend same-mint withdraw/deposit route steps. The route transaction and reusable ALT provisioner use the same standard `POLICY_KEYPAIR`, but ALT mutations remain isolated to the provisioner/cleanup commands. Route execution indexes are selected from the decoded Squads ProgramInteraction policy account when chain preflight is available, instead of trusting stale route metadata.
