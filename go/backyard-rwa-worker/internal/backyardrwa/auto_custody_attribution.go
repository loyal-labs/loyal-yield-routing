...
func (d *Database) observeSharedCustodyAttributionEvidence(ctx context.Context, lease RouteLease, cfg sharedCustodyAttributionConfig, limit int, current *sharedCustodyCurrentOperation) (sharedCustodyAttributionEvidence, error) {
	...
	//
	//   (d) signed rows that lifecycle recovery failed through
	//       MarkExpiredAbsentFailed, sent or not: recovery proved the
	//       signature absent after the finalized block height passed the
	//       wire's last valid height, so it can never land. A wire that had
	//       landed anyway would also break the walk's exact tip balance
	//       match. Refused and dropped AUTO sends left such rows after the
	//       custody origin and held every PYUSD spend (2026-09-24/25).
	//
	...
	candidates, err := tx.Query(ctx, `SELECT operation_id, COALESCE(action,''), COALESCE(last_valid_block_height,0),
		...
		AND NOT (status='manual_recovery' AND COALESCE(action,'') IN ('HOLD_MANUAL_RECOVERY','HOLD_CLEARED')
			...
			AND broadcast_intent_at IS NULL)
		AND NOT (status='failed' AND COALESCE(recovery_reason,'')='signature_absent_after_blockhash_expiry')
