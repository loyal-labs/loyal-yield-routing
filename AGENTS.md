...
For Loyal Hub verification work, treat `crates/loyal-hub-swap-program/verification/loyal_hub_swap.qedspec` as the behavioral source of truth. Treat `crates/loyal-hub-abi/schema/loyal_hub_abi.schema` as the byte-layout source of truth for instruction tags, account positions, data offsets, PDA seed bytes, max-count limits, and byte record lengths. Run `bun run verify:hub-abi-spec-drift` whenever the ABI schema or QEDGen spec changes so overlapping handler accounts and arguments stay aligned. Run `bun run verify:qedgen:check` for spec coverage, `bun run verify:qedgen:proptest` for generated property tests, and `bun run verify:qedgen:probe` for the Pinocchio probe pass. `bun run verify:qedgen` runs the ABI/spec drift gate plus those active QEDGen gates. `bun run verify:qedgen:codegen` regenerates a Quasar scaffold under `target/qedgen` and checks it; it is expected to fail until QEDGen's Quasar codegen emits compilable SPL Token CPI and dynamic PDA support. Do not hand-edit generated files under `target/qedgen` to paper over that failure.

The generated Loyal Hub ABI crate lives in `crates/loyal-hub-abi`. Edit its schema instead of hand-maintaining matching layout constants in the program, SDK, policies, or tests. The action SDK lives in `crates/loyal-actions`. Keep route action construction, route topology, protocol lane configuration, instruction data builders, PDA helpers, and action account planning there so it can move cleanly into Loyal's broader monorepo.

The helper crate lives in `crates/squads-test-harness` and should stay focused on reusable LiteSVM setup, Squads PDA derivation, account seeding helpers, test-only protocol adapters, and deterministic action scenarios. That crate may adapt Loyal Hub instructions into Squads payloads, but should not duplicate Loyal Hub wire data or PDA derivation.
