...
`crates/loyal-worker-supervisor` is a second, narrow test surface under that same rule. Its tests protect contracts that compile while broken and are only observable during an outage: PostgreSQL SQLSTATE and HTTP status mapping to a failure class, whether a class may terminate a process, and the retry-delay bounds. Keep them focused on those decisions; do not add tests that restate the supervisor's struct shape or its telemetry field list.
