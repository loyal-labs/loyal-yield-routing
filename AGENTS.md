...
For Loyal Hub verification work, treat `crates/loyal-hub-swap-program/verification/loyal_hub_swap.qedspec` as the source of truth. Run `bun run verify:qedgen:check` for spec coverage and `bun run verify:qedgen:proptest` for generated property tests. `bun run verify:qedgen:codegen` regenerates a Quasar scaffold under `target/qedgen` and checks it; it is expected to fail until QEDGen's Quasar codegen emits compilable SPL Token CPI and dynamic PDA support. Do not hand-edit generated files under `target/qedgen` to paper over that failure.
