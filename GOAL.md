...
3. One finalized LaserStream path watches every ready smart account and queues
   reconciliation for settings, wallet, vault, policy, supported wallet/vault
   token accounts, and safe-market obligations.
4. Deposit, withdrawal, cleanup, and refund projection uses finalized chain
   evidence and does not require an onboarding attempt or pre-recorded full
   withdrawal row.
...
7. Canonical reads use finalized commitment and `minContextSlot` no lower than
   the triggering update slot.
