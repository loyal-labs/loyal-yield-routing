-- Finalized report failures pay network fees despite atomic instruction rollback.
-- Preserve every prior lifecycle clause. Only exact REPORT_NAV failures with
-- retained finalized receipt, signed-wire identity and booked fee evidence may
-- use this terminal branch. Application settlement holds the route lease and
-- updates both reservation accounting and this proof in the same transaction.
        ...
        (
            ...
            (status = 'failed'
                AND (
                    ...
                    OR (
                        action IS NOT NULL AND action = 'REPORT_NAV'
