...
BEGIN
    ...
    ELSE
        ...
        holding_delta_raw := p_post_confirm_position_amount_raw;
    END IF;

    SELECT event.id
    INTO holding_event_id
    FROM loyal_yield.user_yield_position_holding_events AS event
    ...
    IF FOUND THEN
        IF NOT EXISTS (
            SELECT 1
            FROM loyal_yield.user_yield_position_holding_events AS event
            WHERE event.id = holding_event_id
              AND event.position_id = position_id
              AND event.source_deposit_id = deposit_id
        ) THEN
            RAISE EXCEPTION
    ...
    ELSE
        INSERT INTO loyal_yield.user_yield_position_holding_events (
            ...
            position_id,
            event_type::loyal_yield.user_yield_holding_event_type,
            plan_reserve,
            plan_market,
            plan_liquidity_mint,
            p_post_confirm_position_amount_raw,
        ...
        RETURNING id INTO holding_event_id;
    END IF;

    UPDATE loyal_yield.user_yield_positions
    SET last_holding_event_id = holding_event_id,
        updated_at = now()
    WHERE id = position_id;

    UPDATE loyal_yield.balance_sweep_executions
    SET yield_deposit_id = deposit_id,
        yield_position_id = position_id,
