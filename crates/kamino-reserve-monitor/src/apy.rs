...
mod tests {
    ...
    #[test]
    fn basic_amount_math_handles_fees_and_zero_supply() {
        assert_eq!(total_supply_amount(100.0, 50.0, 10.0, 5.0, 1.0), 134.0);
        assert_eq!(total_supply_amount(1.0, 1.0, 10.0, 0.0, 0.0), 0.0);
        assert_eq!(utilization_ratio(5.0, 0.0), 0.0);
        assert_eq!(utilization_ratio(5.0, 10.0), 0.5);
    }

    #[test]
    fn apr_to_apy_compounds_positive_apr() {
        let apy = apr_to_apy(0.10, 400.0);
        assert!(apy > 0.10);
        assert!(apy < 0.11);
        assert_eq!(apr_to_apy(0.0, 400.0), 0.0);
    }

    #[test]
    fn token_symbols_cover_monitor_stables() {
        for (mint, symbol) in [
            ...
            assert_eq!(token_symbol_for_mint(mint), Some(symbol));
        }
    }
}
