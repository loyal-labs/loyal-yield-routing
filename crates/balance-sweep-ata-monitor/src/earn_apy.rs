...
mod tests {
    ...
    fn at(hour: i64) -> DateTime<Utc> {
        Utc.timestamp_opt(1_800_000_000 + hour * 3600, 0)
            .single()
            .unwrap()
    }

    fn supported(reserve: &str, mint: &str) -> SupportedReserve {
        SupportedReserve {
    ...
    fn update(hour: i64, reserve: &str, mint: &str, supply_apy: f64) -> ReserveUpdate {
        ReserveUpdate {
            observed_at: at(hour),
            reserve: reserve.to_string(),
            liquidity_mint: mint.to_string(),
    ...
    #[test]
    fn hourly_samples_are_rolling_apy_not_elapsed_return_annualization() {
        let main_reserve = KAMINO_MAIN_USDC_RESERVE.to_string();
        let rows = vec![
            update(0, &main_reserve, "USDC", 0.04),
            update(0, "higher", "USDC", 0.10),
        ];
        let snapshots = compute_hourly_snapshots(HourlySnapshotInput {
            end: at(72),
            fee_bps: EARN_APY_FEE_BPS,
            ...
            rows: &rows,
            supported_reserves: &[
                supported(&main_reserve, "USDC"),
                supported("higher", "USDC"),
            ],
        ...
        assert!(snapshots
            .iter()
            .all(|snapshot| snapshot.loyal_apy_bps == 999));
        assert!(snapshots
            .iter()
            .all(|snapshot| snapshot.main_usdc_reserve_apy_bps == 400));
    }

    #[test]
    fn rolling_average_weights_spikes_by_time() {
        let main_reserve = KAMINO_MAIN_USDC_RESERVE.to_string();
        let rows = vec![
            update(0, &main_reserve, "USDC", 0.04),
            update(0, "higher", "USDC", 0.04),
            update(12, "higher", "USDC", 0.20),
            update(18, "higher", "USDC", 0.04),
        ];
        let snapshots = compute_hourly_snapshots(HourlySnapshotInput {
            end: at(24),
            fee_bps: EARN_APY_FEE_BPS,
        ...
        // 18h at 4% and 6h at 20%, then 1 bp fee haircut => 7.99% rolling APY.
        assert_eq!(snapshots[0].loyal_apy_bps, 799);
        assert_eq!(snapshots[0].main_usdc_reserve_apy_bps, 400);
    }
}
