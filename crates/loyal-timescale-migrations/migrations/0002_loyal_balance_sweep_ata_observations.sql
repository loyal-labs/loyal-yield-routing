...
CREATE SCHEMA IF NOT EXISTS loyal;

CREATE SEQUENCE IF NOT EXISTS loyal.balance_sweep_wallet_ata_observation_event_id_seq;

CREATE TABLE IF NOT EXISTS loyal.balance_sweep_wallet_ata_observations (
    event_id BIGINT NOT NULL DEFAULT nextval('loyal.balance_sweep_wallet_ata_observation_event_id_seq'),
...
SELECT create_hypertable(
    'loyal.balance_sweep_wallet_ata_observations',
    'observed_at',
    if_not_exists => TRUE,
...
CREATE INDEX IF NOT EXISTS balance_sweep_wallet_ata_observations_event_id_idx
    ON loyal.balance_sweep_wallet_ata_observations (event_id);
CREATE INDEX IF NOT EXISTS balance_sweep_wallet_ata_observations_wallet_event_idx
    ON loyal.balance_sweep_wallet_ata_observations (wallet_usdc_ata, event_id DESC);
CREATE INDEX IF NOT EXISTS balance_sweep_wallet_ata_observations_target_event_idx
    ON loyal.balance_sweep_wallet_ata_observations (target_id, event_id DESC);
CREATE INDEX IF NOT EXISTS balance_sweep_wallet_ata_observations_slot_desc_idx
    ON loyal.balance_sweep_wallet_ata_observations (slot DESC);
CREATE INDEX IF NOT EXISTS balance_sweep_wallet_ata_observations_time_desc_idx
    ON loyal.balance_sweep_wallet_ata_observations (observed_at DESC);
CREATE INDEX IF NOT EXISTS balance_sweep_wallet_ata_observations_raw_evidence_gin_idx
    ON loyal.balance_sweep_wallet_ata_observations USING GIN (raw_evidence jsonb_path_ops);

CREATE TABLE IF NOT EXISTS loyal.balance_sweep_wallet_ata_observation_dedupe (
...
CREATE INDEX IF NOT EXISTS balance_sweep_wallet_ata_observation_dedupe_event_id_idx
    ON loyal.balance_sweep_wallet_ata_observation_dedupe (event_id);
CREATE INDEX IF NOT EXISTS balance_sweep_wallet_ata_observation_dedupe_ata_slot_idx
    ON loyal.balance_sweep_wallet_ata_observation_dedupe (wallet_usdc_ata, slot);
