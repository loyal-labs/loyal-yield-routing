...
async fn fetch_active_vault_page(
    ...
    let rows = loyal_yield_orchestrator::sqlx::query(
        ...
        SELECT
            ...
            p.id AS policy_id,
            p.cluster AS policy_cluster,
            p.source_commitment AS policy_source_commitment,
            p.finalized_eligible AS policy_finalized_eligible,
            p.authority,
            p.policy_seed,
            p.policy_account,
            p.delegated_signers,
...
fn resolved_vault_from_row(
    ...
    let policy = RoutePolicy {
        ...
        source_commitment: row.try_get("policy_source_commitment")?,
        finalized_eligible: row.try_get("policy_finalized_eligible")?,
