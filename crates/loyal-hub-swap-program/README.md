...
- `config`: stores the admin, hub authorizer, maximum fee in basis points, pause
  flag, and allowed mint list.
- `hub-authority`: owns the hub inventory token accounts and signs hub inventory
  transfers through `invoke_signed`.

The admin can initialize and update config, pause swaps, and withdraw hub
inventory. The hub authorizer must sign each swap. This keeps treasury inventory
approval separate from the delegated smart-account executor.
...
program-derived `hub-authority` PDA. The user vault and hub authorizer must both
...
duplicate mutable account rejection, fee caps, pauses, config updates,
inventory withdrawals, and Jupiter residual fallback.
