...
type RoutePolicyRow = { id: string; policy_account: string; active: boolean };
  ...
  const neon = () =>
    async (strings: TemplateStringsArray, ...values: unknown[]) => {
      ...
      const updated = rows.filter(
        (row) => row.policy_account === account && row.active
      );
...
function createFakeChain(liveAccounts: string[]) {
  const live = new Set(liveAccounts);
  let reads = 0;
  const connection = {
    getAccountInfo: async (key: PublicKey) => {
      reads += 1;
      return live.has(key.toBase58())
        ? { lamports: 2_616_960, data: Buffer.alloc(0), owner: key, executable: false, rentEpoch: 0 }
        : null;
    },
  };
  return { connection: connection as never, reads: () => reads };
}
...
function installPlan(directory: string, missingAccount: string): void {
  writeFileSync(
    join(directory, "plan.json"),
    JSON.stringify({ missingAccount }),
    "utf8"
  );
...
async function simulateTicks(args: {
  ...
  for (let tick = 0; tick < args.ticks; tick += 1) {
    ...
    try {
      ...
      const closed = readClosedRoutePolicyAccount(
        error,
        target.routePolicyAccount
      );
      if (closed) {
        await reconcileClosedRoutePolicy({
          connection: args.connection,
          databaseUrl: DATABASE_URL,
          neon: args.neon,
          policyAccount: new PublicKey(closed),
        });
      }
    }
  }
...
async function scenarioStormReproduced(directory: string): Promise<void> {
  ...
  const { connection } = createFakeChain([]);
  ...
  check("every spawn really ran the binary", spawnCount(directory) === 10, spawnCount(directory));
...
async function scenarioStormStopped(directory: string): Promise<void> {
  ...
  check("no further binary invocations", spawnCount(directory) === 1, spawnCount(directory));
  check("the policy is deactivated", rows[0].active === false);
  check("chain was consulted before writing", reads() === 1, reads());
  check(
  ...
  check(
    ...
    statements[0]?.values[0] === ROUTE_POLICY &&
      /WHERE policy_account = \? AND active/i.test(statements[0]?.sql ?? ""),
    statements[0]
  );
}
...
async function scenarioLivePolicyIsNeverDeactivated(
  ...
  const { neon, statements } = createFakeNeon(rows);
  const { connection } = createFakeChain([ROUTE_POLICY]);

  let outcome: unknown;
  try {
    await dryRun();
  } catch (error) {
    const closed = readClosedRoutePolicyAccount(error, target.routePolicyAccount);
    outcome = await reconcileClosedRoutePolicy({
      connection,
      databaseUrl: DATABASE_URL,
      neon,
      policyAccount: new PublicKey(String(closed)),
    });
  }
...
async function scenarioOtherPolicyIsIgnored(directory: string): Promise<void> {
  ...
  check("the setup policy does not trigger reconciliation", closed === null, closed);
}
...
function scenarioUnrelatedErrorsAreIgnored(): void {
  ...
  for (const [name, message] of [
    ...
    ["obligation missing", "deposit obligation 5YjZj3dk is missing for reserve"],
  ] as const) {
    check(
...
async function scenarioReconciliationIsIdempotent(): Promise<void> {
  ...
  const first = await reconcileClosedRoutePolicy({
    ...
    databaseUrl: DATABASE_URL,
    neon,
    policyAccount: new PublicKey(ROUTE_POLICY),
  });
  ...
  check(
    ...
    (first as { deactivatedPolicyIds?: string[] }).deactivatedPolicyIds?.length === 1,
    first
  );
  check(
    "the second pass deactivates nothing",
    (second as { deactivatedPolicyIds?: string[] }).deactivatedPolicyIds?.length === 0,
    second
  );
}
...
console.error(
  JSON.stringify({
    error: "policy account " + plan.missingAccount + " does not exist",
