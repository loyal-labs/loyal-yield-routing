...
require_fixed_count "$worker_entry" 'save-caches: false' 1 'Pull requests never upload Cargo state'
require_fixed_count "$worker_entry" 'save-caches: true' 2 'Main pushes and the scheduled refresh both fill missing Cargo state'
...
require_text "$workflow" 'uses: actions/cache/save@v4' 'Trusted builds save Cargo state explicitly'
...
require_fixed_count "$workflow" 'uses: actions/cache/save@v4' 2 'Dependency downloads and Cargo target state are saved separately'
...
forbid_pattern "$workflow" 'cargo-target-generation|utc-date=' 'Cargo target cache is not pinned to a calendar generation'
...
require_fixed_count "$workflow" "key: \${{ env.RUST_TARGET_CACHE_PREFIX }}-\${{ matrix.family }}-\${{ hashFiles('Cargo.lock', 'rust-toolchain.toml', 'Cargo.toml', 'crates/*/Cargo.toml') }}-\${{ hashFiles('Cargo.lock', 'rust-toolchain.toml', 'Cargo.toml', 'crates/**', '.sqlx/**', 'scripts/build-rust-image-binaries.sh') }}" 2 'Cargo target restore and save share one family-scoped Rust-source key'
require_fixed_count "$workflow" "hashFiles('Cargo.lock', 'rust-toolchain.toml', 'Cargo.toml', 'crates/**', '.sqlx/**', 'scripts/build-rust-image-binaries.sh')" 2 'Only the restore and save keys carry the Rust source hash'
require_text "$workflow" "'crates/**', '.sqlx/**', 'scripts/build-rust-image-binaries.sh'" 'Cargo target key covers the family package and binary lists, not just crate sources'
require_text "$workflow" "\${{ env.RUST_TARGET_CACHE_PREFIX }}-\${{ matrix.family }}-\${{ hashFiles('Cargo.lock', 'rust-toolchain.toml', 'Cargo.toml', 'crates/*/Cargo.toml') }}-" 'Cargo target restore can roll forward from the latest manifest-compatible snapshot'
...
require_text "$workflow" "steps.cargo-target-cache.outputs.cache-hit != 'true'" 'Cargo target state is saved only when that exact family snapshot is absent'
...
require_fixed_count "$workflow" 'inputs.save-caches' 2 'Only callers that opt in can write Cargo state'
require_fixed_count "$workflow" "github.ref == 'refs/heads/main'" 2 'Both Cargo cache writes are restricted to the trusted branch'
require_fixed_count "$workflow" 'continue-on-error: true' 2 'A failed Cargo cache upload never fails image publication'
