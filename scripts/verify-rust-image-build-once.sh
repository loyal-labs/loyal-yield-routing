...
require_fixed_count "$workflow" "key: \${{ env.RUST_TARGET_CACHE_PREFIX }}-\${{ matrix.family }}-\${{ hashFiles('Cargo.lock', 'rust-toolchain.toml', 'Cargo.toml', 'crates/*/Cargo.toml') }}-\${{ hashFiles('Cargo.lock', 'rust-toolchain.toml', 'Cargo.toml', 'crates/**', '.sqlx/**') }}" 2 'Cargo target restore and save share one family-scoped Rust-source key'
require_fixed_count "$workflow" "hashFiles('Cargo.lock', 'rust-toolchain.toml', 'Cargo.toml', 'crates/**', '.sqlx/**')" 2 'Only the restore and save keys carry the Rust source hash'
