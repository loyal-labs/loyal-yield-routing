...
migration="crates/loyal-yield-store/migrations/0045_atomic_autodeposit_finalization.sql"
[[ -f "$migration" ]] || {
  echo "FAIL: missing migration 0045 atomic autodeposit finalization" >&2
  exit 1
}

rg -q 'version: 45,' crates/loyal-yield-orchestrator/src/bin/yield-migrations.rs || {
  echo "FAIL: production migration runner does not register migration 45" >&2
  exit 1
}
...
echo PASS_AUTODEPOSIT_ATOMIC_FINALIZATION
